...
export default function LaundryYaluApp() {
  ...
  const whatsappMessage = encodeURIComponent(
    `Hello LaundryYalu, I want to book a laundry pickup.\n\n` +
      `Name: ${customerName || "Not added"}\n` +
      `Phone: ${phone || "Not added"}\n` +
      `Address: ${address || "Not added"}\n` +
      `Pickup Date: ${pickupDate || "Not selected"}\n` +
      `Pickup Time: ${pickupTime}\n\n` +
      `Service: ${service.name}\n` +
      `Load: ${load.name} - ${load.items} - ${load.weight}\n` +
      `Add-ons:\n${selectedAddOnLabels.join("\n") || "None"}\n` +
      `Delivery Zone: ${zone.name} (${zone.distance})\n\n` +
      `Laundry Package: ${formatLKR(load.price)}\n` +
      `Add-ons: ${formatLKR(addOnTotal)}\n` +
      `Pickup Estimate: ${formatLKR(finalDeliveryFee)}\n` +
      `Estimated Total: ${formatLKR(finalTotal)}\n\n` +
      `Notes: ${notes || "None"}`
  );
